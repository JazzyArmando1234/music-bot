...
var SoundCloudExtractor = class extends import_discord_player.BaseExtractor {
  async validate(query, type) {
    if (typeof query !== "string")
      return false;
    return [import_discord_player.QueryType.SOUNDCLOUD, import_discord_player.QueryType.SOUNDCLOUD_PLAYLIST, import_discord_player.QueryType.SOUNDCLOUD_SEARCH, import_discord_player.QueryType.SOUNDCLOUD_TRACK].some((r) => r === type);
  }
...
function getHTML(link) {
  return (0, import_node_fetch.default)(link, {
    headers: {
...
var AppleMusic = class {
  ...
  static async getSongInfo(link) {
    if (!import_discord_player2.QueryResolver.regex.appleMusicSongRegex.test(link)) {
      return null;
    }
  ...
  static async getPlaylistInfo(link) {
    if (!import_discord_player2.QueryResolver.regex.appleMusicPlaylistRegex.test(link)) {
      return null;
    }
  ...
  static async getAlbumInfo(link) {
    if (!import_discord_player2.QueryResolver.regex.appleMusicAlbumRegex.test(link)) {
      return null;
    }
    const res = await getHTML(link);
    if (!res)
...
var getFetch = typeof fetch !== "undefined" ? fetch : typeof require !== "undefined" ? import_discord_player3.Util.require("undici")?.fetch || import_discord_player3.Util.require("node-fetch") : async (...params) => {
  let dy;
  if (dy = await import_discord_player3.Util.import("undici").module) {
    return (dy.fetch || dy.default.fetch)(...params);
  } else if (dy = await import_discord_player3.Util.import("node-fetch").module) {
    return (dy.fetch || dy.default)(...params);
  } else {
    throw new Error("No fetch lib found");
  }
};
var _YoutubeExtractor = class extends import_discord_player3.BaseExtractor {
  async activate() {
    let lib;
    for (const ytlib of YouTubeLibs) {
      lib = (await import_discord_player3.Util.import(ytlib)).module;
      if (!lib)
        continue;
      lib = lib.default || lib;
    ...
    if (lib) {
      const isYtdl = ["ytdl-core", "@distube/ytdl-core"].some((lib2) => lib2 === this._ytLibName);
    ...
    this._spotify = (0, import_spotify_url_info.default)(getFetch);
  }
  async validate(query, type) {
    if (typeof query !== "string")
      return false;
    return [
      import_discord_player3.QueryType.YOUTUBE,
      import_discord_player3.QueryType.YOUTUBE_PLAYLIST,
      import_discord_player3.QueryType.YOUTUBE_SEARCH,
      import_discord_player3.QueryType.YOUTUBE_VIDEO,
      import_discord_player3.QueryType.SPOTIFY_ALBUM,
      import_discord_player3.QueryType.SPOTIFY_PLAYLIST,
      import_discord_player3.QueryType.SPOTIFY_SONG,
      import_discord_player3.QueryType.APPLE_MUSIC_ALBUM,
      import_discord_player3.QueryType.APPLE_MUSIC_PLAYLIST,
      import_discord_player3.QueryType.APPLE_MUSIC_SONG,
      import_discord_player3.QueryType.AUTO
    ].some((r) => r === type);
  }
  async handle(query, context) {
    if (_YoutubeExtractor.validateURL(query))
      context.type = import_discord_player3.QueryType.YOUTUBE_VIDEO;
    switch (context.type) {
      case import_discord_player3.QueryType.YOUTUBE_PLAYLIST: {
        ...
        if (!ytpl)
          return this.emptyResponse();
        const playlist = new import_discord_player3.Playlist(this.context.player, {
          title: ytpl.title,
        ...
        playlist.tracks = ytpl.videos.map(
          (video) => new import_discord_player3.Track(this.context.player, {
            title: video.title,
      ...
      case import_discord_player3.QueryType.YOUTUBE_VIDEO: {
        const id = /[a-zA-Z0-9-_]{11}/.exec(query);
        if (!id?.[0])
          return this.emptyResponse();
        const video = await import_youtube_sr.YouTube.getVideo(`https://www.youtube.com/watch?v=${id}`, context.requestOptions).catch(import_discord_player3.Util.noop);
        if (!video)
          return this.emptyResponse();
        ...
        return {
          playlist: null,
          tracks: [
            new import_discord_player3.Track(this.context.player, {
              title: video.title,
              description: video.description,
              author: video.channel?.name,
              url: video.url,
      ...
      case import_discord_player3.QueryType.SPOTIFY_SONG: {
        const spotifyData = await this._spotify.getData(query, context.requestOptions).catch(import_discord_player3.Util.noop);
        if (!spotifyData)
          return { playlist: null, tracks: [] };
        const spotifyTrack = new import_discord_player3.Track(this.context.player, {
          ...
          duration: import_discord_player3.Util.buildTimeCode(import_discord_player3.Util.parseMS(spotifyData.duration ?? spotifyData.maxDuration ?? 0)),
          views: 0,
          requestedBy: context.requestedBy,
          source: "spotify",
          queryType: context.type
      ...
      case import_discord_player3.QueryType.SPOTIFY_PLAYLIST: {
        const spotifyPlaylist = await this._spotify.getData(query, context.requestOptions).catch(import_discord_player3.Util.noop);
        if (!spotifyPlaylist)
          return { playlist: null, tracks: [] };
        const playlist = new import_discord_player3.Playlist(this.context.player, {
        ...
        playlist.tracks = spotifyPlaylist.trackList.map((m) => {
          const data = new import_discord_player3.Track(this.context.player, {
            title: m.title ?? "",
            description: m.title ?? "",
            author: m.subtitle ?? "Unknown Artist",
            url: m.uid ? `https://open.spotify.com/tracks/${m.uid}` : query,
            thumbnail: "https://www.scdn.co/i/_global/twitter_card-default.jpg",
            duration: import_discord_player3.Util.buildTimeCode(import_discord_player3.Util.parseMS(m.duration)),
            views: 0,
            requestedBy: context.requestedBy,
            playlist,
            source: "spotify",
      ...
      case import_discord_player3.QueryType.SPOTIFY_ALBUM: {
        const album = await this._spotify.getData(query, context.requestOptions).catch(import_discord_player3.Util.noop);
        if (!album)
          return { playlist: null, tracks: [] };
        const playlist = new import_discord_player3.Playlist(this.context.player, {
      ...
      case import_discord_player3.QueryType.APPLE_MUSIC_ALBUM: {
        ...
        playlist.tracks = info.tracks.map(
          (m) => new import_discord_player3.Track(this.context.player, {
            author: m.artist.name,
            description: m.title,
            duration: typeof m.duration === "number" ? import_discord_player3.Util.buildTimeCode(import_discord_player3.Util.parseMS(m.duration)) : m.duration,
      ...
      case import_discord_player3.QueryType.APPLE_MUSIC_PLAYLIST: {
      ...
      case import_discord_player3.QueryType.APPLE_MUSIC_SONG: {
        ...
        const track = new import_discord_player3.Track(this.context.player, {
          ...
          description: info.title,
          duration: typeof info.duration === "number" ? import_discord_player3.Util.buildTimeCode(import_discord_player3.Util.parseMS(info.duration)) : info.duration,
          thumbnail: info.thumbnail,
          title: info.title,
          url: info.url,
          views: 0,
  ...
  async _makeYTSearch(query, context) {
    ...
    return res.map((video) => {
      video.source = "youtube";
      return new import_discord_player3.Track(this.context.player, {
        title: video.title,
        description: video.description,
        author: video.channel?.name,
        url: video.url,
  ...
  async stream(info) {
    ...
    if (info.queryType === "spotifySong" || info.queryType === "appleMusicSong") {
      if (_YoutubeExtractor.validateURL(info.raw.url))
        ...
        const _url = await import_youtube_sr.YouTube.searchOne(`${info.title} ${info.author}`, "video").then((r) => r.url).catch(import_discord_player3.Util.noop);
        if (!_url)
...
YoutubeExtractor.identifier = "com.discord-player.ysaextractor";
...
// src/extractors/VimeoExtractor.ts
var VimeoExtractor = class extends import_discord_player4.BaseExtractor {
  async validate(query, type) {
    if (typeof query !== "string")
      return false;
    return [import_discord_player4.QueryType.VIMEO].some((r) => r === type);
  }
  async handle(query, context) {
    switch (context.type) {
      case import_discord_player4.QueryType.VIMEO: {
        const trackInfo = await Vimeo.getInfo(
          query.split("/").filter((x) => !!x).pop()
        ).catch(import_discord_player4.Util.noop);
        if (!trackInfo)
          return this.emptyResponse();
        const track = new import_discord_player4.Track(this.context.player, {
          title: trackInfo.title,
          url: trackInfo.url,
          duration: import_discord_player4.Util.buildTimeCode(import_discord_player4.Util.parseMS(trackInfo.duration || 0)),
  ...
  async stream(info) {
    ...
    const track = await Vimeo.getInfo(info.url).catch(import_discord_player4.Util.noop);
...
var ReverbnationExtractor = class extends import_discord_player5.BaseExtractor {
  async validate(query, type) {
    if (typeof query !== "string")
      return false;
    return [import_discord_player5.QueryType.REVERBNATION].some((r) => r === type);
  }
  async handle(query, context) {
    switch (context.type) {
      case import_discord_player5.QueryType.REVERBNATION: {
        const trackInfo = await import_reverbnation_scraper.default.getInfo(query).catch(import_discord_player5.Util.noop);
        if (!trackInfo)
          return this.emptyResponse();
  ...
  async stream(info) {
    ...
    const track = await import_reverbnation_scraper.default.getInfo(info.url).catch(import_discord_player5.Util.noop);
...
var AttachmentExtractor = class extends import_discord_player6.BaseExtractor {
  async validate(query, type) {
    if (typeof query !== "string")
      return false;
    return [import_discord_player6.QueryType.ARBITRARY, import_discord_player6.QueryType.FILE].some((r) => r === type);
  }
  async handle(query, context) {
    switch (context.type) {
      case import_discord_player6.QueryType.ARBITRARY: {
        ...
        const track = new import_discord_player6.Track(this.context.player, {
          title: trackInfo.title,
          url: trackInfo.url,
          duration: import_discord_player6.Util.buildTimeCode(import_discord_player6.Util.parseMS(trackInfo.duration)),
          description: trackInfo.description,
          thumbnail: trackInfo.thumbnail,
          views: 0,
      ...
      case import_discord_player6.QueryType.FILE: {
