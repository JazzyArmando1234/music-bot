...
var getFetch = typeof fetch !== "undefined" ? fetch : typeof __require !== "undefined" ? Util2.require("undici")?.fetch || Util2.require("node-fetch") : async (...params) => {
  let dy;
  if (dy = await Util2.import("undici").module) {
    return (dy.fetch || dy.default.fetch)(...params);
  } else if (dy = await Util2.import("node-fetch").module) {
...
var _YoutubeExtractor = class extends BaseExtractor2 {
  async activate() {
    ...
    for (const ytlib of YouTubeLibs) {
      lib = (await Util2.import(ytlib)).module;
      if (!lib)
        continue;
      lib = lib.default || lib;
  ...
  async validate(query, type) {
    ...
    return [
      ...
      QueryType2.SPOTIFY_ALBUM,
      QueryType2.SPOTIFY_PLAYLIST,
      QueryType2.SPOTIFY_SONG,
      QueryType2.APPLE_MUSIC_ALBUM,
      QueryType2.APPLE_MUSIC_PLAYLIST,
      QueryType2.APPLE_MUSIC_SONG,
      QueryType2.AUTO
    ].some((r) => r === type);
  }
  async handle(query, context) {
    ...
    switch (context.type) {
      ...
      case QueryType2.YOUTUBE_VIDEO: {
        ...
        if (!id?.[0])
          return this.emptyResponse();
        const video = await YouTube.getVideo(`https://www.youtube.com/watch?v=${id}`, context.requestOptions).catch(Util2.noop);
        if (!video)
          return this.emptyResponse();
      ...
      case QueryType2.SPOTIFY_SONG: {
        const spotifyData = await this._spotify.getData(query, context.requestOptions).catch(Util2.noop);
        if (!spotifyData)
          return { playlist: null, tracks: [] };
        const spotifyTrack = new Track2(this.context.player, {
          ...
          duration: Util2.buildTimeCode(Util2.parseMS(spotifyData.duration ?? spotifyData.maxDuration ?? 0)),
          views: 0,
          requestedBy: context.requestedBy,
          source: "spotify",
          queryType: context.type
      ...
      case QueryType2.SPOTIFY_PLAYLIST: {
        const spotifyPlaylist = await this._spotify.getData(query, context.requestOptions).catch(Util2.noop);
        if (!spotifyPlaylist)
          return { playlist: null, tracks: [] };
        const playlist = new Playlist2(this.context.player, {
      ...
      case QueryType2.SPOTIFY_ALBUM: {
        const album = await this._spotify.getData(query, context.requestOptions).catch(Util2.noop);
        if (!album)
          return { playlist: null, tracks: [] };
        const playlist = new Playlist2(this.context.player, {
      ...
      case QueryType2.APPLE_MUSIC_ALBUM: {
        ...
        playlist.tracks = info.tracks.map(
          (m) => new Track2(this.context.player, {
            author: m.artist.name,
            description: m.title,
            duration: typeof m.duration === "number" ? Util2.buildTimeCode(Util2.parseMS(m.duration)) : m.duration,
      ...
      case QueryType2.APPLE_MUSIC_PLAYLIST: {
      ...
      case QueryType2.APPLE_MUSIC_SONG: {
        ...
        const track = new Track2(this.context.player, {
          ...
          description: info.title,
          duration: typeof info.duration === "number" ? Util2.buildTimeCode(Util2.parseMS(info.duration)) : info.duration,
          thumbnail: info.thumbnail,
          title: info.title,
          url: info.url,
          views: 0,
  ...
  async stream(info) {
    ...
    if (info.queryType === "spotifySong" || info.queryType === "appleMusicSong") {
      if (_YoutubeExtractor.validateURL(info.raw.url))
        ...
        const _url = await YouTube.searchOne(`${info.title} ${info.author}`, "video").then((r) => r.url).catch(Util2.noop);
        if (!_url)
...
export {
  ...
  SoundCloudExtractor,
  VimeoExtractor,
  YoutubeExtractor as YSAExtractor,
